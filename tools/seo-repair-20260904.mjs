...
const BOARD = {
  insights: {
    label: 'Insights',
    title: '여러 서비스에서 반복되는 문제',
    lead: '서로 다른 사용자 문의와 장애 사례를 묶어, 반복해서 나타나는 문제와 그 원인을 정리합니다.'
  },
  cases: {
    label: 'Cases',
    title: '실제 문제를 끝까지 따라간 기록',
    lead: '한 서비스에서 실제로 막힌 흐름을 따라가며, 어디서 문제가 생겼고 무엇을 확인해야 하는지 정리합니다.'
  },
  guides: {
    label: 'Guides',
    title: '반복 사례에서 뽑은 실무 가이드',
    lead: '여러 실제 사례에서 반복된 문제를 바탕으로, 설계·운영·검수 때 바로 확인할 기준을 정리합니다.'
  }
};
...
function relatedServicesFor(rel) {
  ...
  if (/deploy|api|callback/.test(s)) return [
    ['/services/web-app-development/', '웹·앱 개발'],
    ['/solutions/api-integration-development/', 'API 연동 개발']
  ];
  if (/signup|login|account|state|list|pagination/.test(s)) return [
    ['/services/web-app-development/', '웹·앱 개발'],
    ['/solutions/admin-system-development/', '관리자 시스템 개발']
  ];
  if (/delete|recovery|recover/.test(s)) return [
    ['/services/software-rebuild/', '기존 소프트웨어 개선·재구축'],
    ['/solutions/admin-system-development/', '관리자 시스템 개발']
  ];
  return [
    ['/services/web-app-development/', '웹·앱 개발'],
...
for (const type of Object.keys(BOARD)) {
  ...
  const schema = {
    ...
    '@type': 'CollectionPage',
    name: `${b.label} | SuaveForge`,
    url: `https://suaveforge.com/${type}/`,
    description: b.lead,
    mainEntity: {
      '@type': 'ItemList',
      itemListElement: items.map((item, i) => ({
        ...
        position: i + 1,
        name: item.title,
        url: `https://suaveforge.com/${type}/${item.slug}/`
      }))
    }
  };
...
function setLastmod(xml, url, date) {
  const escaped = url.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const re = new RegExp(`(<url><loc>${escaped}<\\/loc><lastmod>)[^<]+(<\\/lastmod>)`);
...
for (const name of ['sitemap.xml', 'sitemap-services.xml']) {
  ...
  for (const url of [
    'https://suaveforge.com/',
    'https://suaveforge.com/services/automation-development/',
    'https://suaveforge.com/services/software-rebuild/',
    'https://suaveforge.com/insights/',
    'https://suaveforge.com/cases/',
    'https://suaveforge.com/guides/'
  ]) xml = setLastmod(xml, url, TODAY);
  for (const type of ['insights', 'cases', 'guides']) {
    for (const item of manifest[type] || []) xml = setLastmod(xml, `https://suaveforge.com/${type}/${item.slug}/`, TODAY);
  }
