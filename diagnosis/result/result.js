(()=>{
  ...
  async function initCharts(all,cats,scores,lh){
    ...
    if(tree){const chart=ech.init(tree);charts.push(chart);const data=cats.map(x=>({name:x.category,value:x.count,max:x.max,avg:x.avg,itemStyle:{color:x.max>=85?'#ef5b53':x.max>=65?'#e7a437':'#5578e9'}}));chart.setOption({animationDuration:1200,tooltip:{trigger:'item',backgroundColor:'#11151c',borderWidth:0,textStyle:{color:'#fff',fontSize:10},formatter:p=>`<b>${esc(p.name)}</b><br/>발견 ${p.value}건 · 최고 ${p.data.max} · 평균 ${p.data.avg}`},series:[{type:'treemap',data,roam:false,nodeClick:false,breadcrumb:{show:false},visibleMin:1,label:{show:true,formatter:p=>`{name|${p.name}}\n{meta|${p.value}건 · 최고 ${p.data.max}}`,rich:{name:{fontSize:14,fontWeight:900,color:'#fff',lineHeight:22},meta:{fontSize:9,color:'rgba(255,255,255,.8)',lineHeight:16}}},upperLabel:{show:false},itemStyle:{borderColor:'#fff',borderWidth:4,gapWidth:4,borderRadius:10},emphasis:{itemStyle:{shadowBlur:24,shadowColor:'rgba(16,18,22,.22)'}}}]});chart.on('click',p=>p.name&&activateCategory(p.name))}
