(()=>{
  ...
  const stages=[
    ...
    {key:'lighthouse',label:'성능 교차 측정',sub:'성능 · SEO · 접근성',match:['lighthouse']},
    {key:'finalize',label:'결과 우선순위 정리',sub:'근거 기반 Top 3 구성',match:['finalize','completed']}
  ];
  const signalDefs=[['desktop','데스크톱','1440px'],['mobile','모바일','390×844'],['seo','검색 구조','문서 확인'],['accessibility','접근성','자동 검사'],['security','보안','응답 확인'],['lighthouse','성능 측정','핵심 지표']];
  ...
  const confidenceLabel=v=>v==='CONFIRMED'?'확인됨':v==='STRONG_INFERENCE'?'근거 충분':v==='NEEDS_REVIEW'?'추가 확인':'확인';
  ...
  const locations=x=>Array.isArray(x.locations)&&x.locations.length?`<div class="locations"><b>확인 위치</b>${x.locations.slice(0,8).map(v=>`<code>${esc(v)}</code>`).join('')}</div>`:'';

  function progressShell(){
    if(main.querySelector('.audit-live'))return;
    main.innerHTML=`<section class="audit-live" aria-live="polite">
      <div class="audit-kicker"><span class="live-dot"></span><span>진단 진행 중</span><span class="audit-id">#${esc(id.slice(0,8))}</span></div>
      <div class="audit-grid">
        <div class="audit-visual">
          <div class="audit-command">
            <div class="audit-copy">
              <div class="audit-state-line"><span class="state-pulse"></span><b id="stageBadge">진단 시작</b><span id="stageCounter">01 / ${String(stages.length).padStart(2,'0')}</span></div>
          ...
          <div class="audit-telemetry">
            <div class="telemetry-head"><div><span class="telemetry-kicker">실시간 진행</span><b>실시간 진단 진행 신호</b></div><code id="targetHost">target</code></div>
            <div class="telemetry-grid">
              ...
              <div class="scan-stage" aria-hidden="true">
                <div class="browser-frame">
                  <div class="browser-top"><i></i><i></i><i></i><span id="browserTarget">target</span><em>자동 브라우저 검사</em></div>
        ...
        <aside class="audit-pipeline">
          <div class="pipeline-head"><div><span>검사 단계</span><b>검사 실행 흐름</b></div><strong><span id="currentStepNo">01</span><small>/${String(stages.length).padStart(2,'0')}</small></strong></div>
          ...
          <div class="pipeline-foot"><span class="pipeline-led"></span><div><b id="pipelineState">대기열 확인 중</b><small>실제 검사 상태를 기준으로 표시합니다.</small></div></div>
        </aside>
      </div>
  ...
  function actionCard(x){
    ...
    return `<article class="action-card" ${Number.isInteger(x._index)&&x._index>=0?`data-jump-finding="${x._index}"`:``}><div class="action-score"><strong>${p}</strong><span>우선순위</span></div><div class="action-body"><div class="action-meta"><span class="tag ${priorityClass(x)}">${priorityLabel(x)}</span><span class="tag category">${esc(x.category)}</span><span class="tag confidence">${esc(confidenceLabel(x.confidence))}</span></div><h3>${esc(x.title)}</h3><p>${esc(x.userImpact)}</p><div class="action-evidence"><div><b>실제 근거</b>${esc(x.evidence)}</div><div><b>개선 방법</b>${esc(x.improvement)}</div></div>${locations(x)}<div class="difficulty-line"><span>수정 난이도</span><b>${esc(difficulty(x))}</b><span>· 예상 작업 시간은 실제 수정 범위 확인 후 안내</span></div></div></article>`;
  }
  ...
  function render(d){
    ...
    const vital=(key,label,value,help,good,needs,poor)=>{const [state,klass]=metricState(key,value);return `<article class="vital-card" data-vital="${key}" data-tippy-content="${esc(help)} · 기준: 좋음 ${esc(good)}, 개선 필요 ${esc(needs)}, 나쁨 ${esc(poor)}"><div class="vital-top"><span class="vital-name metric-help-tip">${label}</span><strong class="vital-value">${metricValue(key,value)}</strong><span class="vital-state ${klass}">${state}</span></div><div class="vital-chart" id="vital-${key}"></div><div class="vital-foot"><span>좋음</span><span>개선 필요</span><span>나쁨</span></div></article>`};
    main.innerHTML=`
      <section class="result-head report-reveal"><div><div class="eyebrow">진단 결과 · 확인된 근거 기반</div><h1>${esc(d.site_title||d.hostname)}</h1><p class="result-url"><span>${esc(d.normalized_url)}</span>${processing!==null?`<span class="dot">•</span><span>완료 ${processing}초</span>`:''}<span class="dot">•</span><span>#${esc(id.slice(0,8))}</span></p></div><div class="result-head-side"><b class="count-up" data-value="${all.length}">${all.length}</b><span>발견 항목</span></div></section>
      <section class="executive report-reveal"><div class="executive-inner"><div class="executive-copy"><div class="eyebrow">핵심 요약</div><h2>${esc(summary)}</h2><p>점수보다 먼저 실제 증거와 영향도를 봅니다. 아래 시각화는 진단에서 수집된 값만 사용하며, 임의 점수나 고정 시간 견적을 만들지 않습니다.</p></div><div class="executive-stats"><div class="executive-stat is-alert"><b class="count-up" data-value="${immediate}">${immediate}</b><span>즉시 수정</span></div><div class="executive-stat"><b class="count-up" data-value="${recommended}">${recommended}</b><span>권장 수정</span></div><div class="executive-stat ${runtimeErrors?'is-alert':'is-ok'}"><b class="count-up" data-value="${runtimeErrors}">${runtimeErrors}</b><span>실행 오류</span></div><div class="executive-stat"><b>${m.performance?.resourceCount??'—'}</b><span>불러온 리소스</span></div><div class="executive-stat"><b>${bytes(m.performance?.transferSize)}</b><span>전송량</span></div><div class="executive-stat ${m.security?.mixedContent?'is-alert':'is-ok'}"><b>${m.security?.mixedContent??'—'}</b><span>혼합 콘텐츠</span></div></div></div></section>
      ...
      <section class="report-section report-reveal" id="scores"><div class="section-head"><div><div class="section-kicker">01 · 성능 상태</div><h2>점수가 아니라 상태를 읽는 계기판</h2></div><p>성능 점수와 핵심 체감 지표를 함께 보여줍니다. 각 수치에 마우스를 올리면 기준을 확인할 수 있습니다.</p></div><div class="metric-cockpit"><div class="score-orbit-panel"><div class="panel-label"><h3>성능·검색·접근성 점수</h3><span>0—100</span></div><div id="scoreOrbitChart" class="score-orbit-chart"></div><div class="score-legend">${scoreLegend('성능',perf,'#f2a01d')}${scoreLegend('SEO',seo,'#24b978')}${scoreLegend('접근성',a11y,'#54d7a2')}</div></div><div class="vitals-panel"><div class="vitals-head"><div><h3>핵심 성능 지표</h3><p>각 값이 기준선의 어느 구간에 있는지 실제 측정 위치로 표시합니다.</p></div><span class="report-version">측정 결과</span></div><div class="vital-stack">${vital('lcp','LCP',lh.lcp,'가장 큰 콘텐츠가 보이는 시점','≤ 2.5s','2.5–4.0s','> 4.0s')}${vital('tbt','TBT',lh.tbt,'메인 스레드가 입력을 막은 누적 시간','≤ 200ms','200–600ms','> 600ms')}${vital('cls','CLS',lh.cls,'예상치 못한 레이아웃 이동량','≤ 0.10','0.10–0.25','> 0.25')}${vital('fcp','FCP',lh.fcp,'첫 콘텐츠가 화면에 그려지는 시점','≤ 1.8s','1.8–3.0s','> 3.0s')}</div></div></div></section>
      <section class="report-section report-reveal" id="discovery"><div class="section-head"><div><div class="section-kicker">02 · 검색·답변 노출</div><h2>SEO · AEO · GEO를 따로 봅니다</h2></div><p>SEO·AEO·GEO를 각각 다른 기준으로 확인합니다. 실제로 확인된 신호만 표시하고, 확인하지 못한 항목은 점수로 만들지 않습니다.</p></div><div class="discovery-grid">${discoveryCard('seo','검색 노출 기반','색인·문서 구조·검색 결과 메타데이터를 확인합니다.',seoRows,seo===null?'N/A':`${Math.round(seo)} / 100`)}${discoveryCard('aeo','답변 추출 준비도','질문과 직접 답변, 구조화된 설명 단위를 확인합니다.',aeoRows,`${aeoAnswerCount}/${aeoQuestionCount||'—'} 직접답변`)}${discoveryCard('geo','생성형 검색 해석 신호','엔티티·출처·AI 접근 정책을 확인합니다.',geoRows,`${Number(geo.entitySchemaCount)||0} 엔티티`)}</div></section>
      <section class="report-section report-reveal" id="priority" data-density="${all.length<=3?'sparse':'normal'}"><div class="section-head"><div><div class="section-kicker">03 · 수정 우선순위</div><h2>어디부터 고쳐야 하는지 한 번에</h2></div><p>우선순위와 문제 분포를 함께 보여줍니다. 항목을 선택하면 해당 상세 진단으로 이동합니다.</p></div><div class="insight-grid"><div class="insight-card dark"><div class="insight-card-head"><div><h3>우선순위</h3><p>상위 ${Math.min(8,all.length)}개 · 진단 근거와 영향도를 기준으로 정렬</p></div><span class="report-version">항목 선택</span></div><div class="impact-chart" id="impactChart"></div></div><div class="insight-card dark" data-secondary-chart><div class="insight-card-head"><div><h3>문제 분포</h3><p>분야별 우선순위와 근거량을 함께 표시합니다.</p></div><span class="report-version">항목 연동</span></div><div class="category-chart" id="issueMapChart"></div></div></div><aside class="issue-spotlight" id="issueSpotlight" aria-live="polite"><div class="issue-spotlight__pulse"></div><div><span>선택한 문제</span><strong>${esc(top[0]?.title||all[0]?.title||'진단 항목을 선택하세요')}</strong><p>${esc(top[0]?.evidence||all[0]?.evidence||'항목을 선택하면 관련 근거와 상세 진단을 확인할 수 있습니다.')}</p></div><b>${Math.round(Number(top[0]?.priority||all[0]?.priority)||0)}</b></aside><div class="action-list">${top.length?top.map(x=>actionCard({...x,_index:all.findIndex(y=>y.title===x.title&&y.category===x.category)})).join(''):'<article class="action-card"><div class="action-score"><strong>0</strong><span>우선순위</span></div><div class="action-body"><h3>즉시 우선순위로 분류된 문제가 많지 않습니다.</h3><p>상세 진단과 공개 검사 범위를 확인해 주세요.</p></div></article>'}</div></section>
      <section class="report-section report-reveal" id="matrix"><div class="section-head"><div><div class="section-kicker">04 · 분야별 영향</div><h2>분야별 상태와 영향 밀도</h2></div><p>분야별 발견 건수와 우선순위를 함께 보여주고, 문제마다 확인 지점부터 수정 위치까지 연결해 보여줍니다.</p></div><div class="insight-card dark" style="margin-bottom:14px"><div class="insight-card-head"><div><h3>${all.length<=3?'문제 해결 경로':'분야별 문제 관계'}</h3><p>${all.length<=3?'확인 지점 → 영향 → 수정 위치 순서로 연결합니다.':'분야와 대표 문제를 영향 강도에 따라 연결해 보여줍니다.'}</p></div><span class="report-version">${all.length<=3?'항목 선택':'이동 · 선택'}</span></div><div class="chart" id="categoryTreemap"></div></div><div class="matrix-card"><table class="matrix-table"><thead><tr><th>분야</th><th>발견</th><th>최고 우선순위</th><th>평균</th><th>상태</th></tr></thead><tbody>${cats.length?cats.map(x=>`<tr data-category-row="${esc(x.category)}"><td>${esc(x.category)}</td><td class="matrix-count">${x.count}</td><td><div class="matrix-bar" aria-label="최고 우선순위 ${x.max}"><i style="width:${Math.max(2,x.max)}%"></i></div></td><td>${x.avg}</td><td><span class="tag ${x.max>=85?'immediate':x.max>=65?'recommended':'reference'}">${x.max>=85?'즉시 수정':x.max>=65?'권장 수정':'참고'}</span></td></tr>`).join(''):'<tr><td colspan="5">발견 항목 없음</td></tr>'}</tbody></table></div></section>
      ${Object.keys(shots).length?`<section class="report-section report-reveal" id="evidence"><div class="section-head"><div><div class="section-kicker">05 · 화면 근거</div><h2>실제 화면으로 확인</h2></div><p>진단 중 캡처한 실제 화면입니다. 좌우로 이동하거나 각 화면을 선택해 원본을 확인할 수 있습니다.</p></div><div class="evidence-shell"><div class="evidence-toolbar"><h3>화면 근거</h3><span>이동 · 원본 보기</span></div><div class="swiper evidence-swiper"><div class="swiper-wrapper">${[['desktop','데스크톱 전체 캡처'],['mobile','모바일 전체 캡처'],['annotated','문제 위치 표시'],['improvement','개선 예시']].filter(([k])=>shots[k]).map(([k,l],i)=>`<div class="swiper-slide"><a href="${esc(shots[k])}" target="_blank" rel="noopener" class="evidence-shot" data-evidence-index="${i}"><img src="${esc(shots[k])}" alt="${l}" loading="lazy"><span>${l}</span></a></div>`).join('')}</div><div class="swiper-scrollbar"></div></div></div></section>`:''}
      <section class="report-section report-reveal" id="details"><div class="section-head"><div><div class="section-kicker">06 · 상세 진단</div><h2>상세 진단</h2></div><p>각 문제를 근거 → 사용자 영향 → 사업 영향 → 개선 방법 순서로 확인할 수 있습니다.</p></div><div class="detail-tools">${categoryOptions.map((c,i)=>`<button type="button" class="filter-chip ${i===0?'active':''}" data-filter="${esc(c)}">${esc(c)}</button>`).join('')}</div><div class="details">${all.map((x,i)=>`<details class="detail" id="finding-${i}" data-finding-index="${i}" data-category="${esc(x.category||'기타')}" ${i===0?'open':''}><summary><span class="detail-priority">${Math.round(Number(x.priority)||0)}</span><span class="detail-title"><b>${esc(x.title)}</b><small>${esc(x.category)} · ${priorityLabel(x)} · ${esc(confidenceLabel(x.confidence))}</small></span></summary><div class="detail-grid"><div><b>실제 근거</b>${esc(x.evidence)}</div><div><b>사용자 영향</b>${esc(x.userImpact)}</div><div><b>사업 영향</b>${esc(x.businessImpact)}</div><div><b>개선 방법</b>${esc(x.improvement)}</div><div><b>수정 난이도</b>${esc(difficulty(x))}</div><div><b>재개발 판단</b>${esc(x.redevelopment)}</div>${Array.isArray(x.locations)&&x.locations.length?`<div class="detail-locations"><b>문제 위치</b>${x.locations.slice(0,12).map(v=>`<code>${esc(v)}</code>`).join('')}</div>`:''}</div></details>`).join('')}</div></section>
      <section class="report-section next-contact" id="next"><div class="section-head"><div><div class="section-kicker">07 · 다음 단계</div><h2>결과를 이어서 확인하기</h2></div><p>진단 결과를 보존하거나 공개 페이지 밖의 실제 사용자 흐름까지 직접 확인합니다.</p></div><div class="forms"><form class="formbox" id="mailForm"><h3>이메일로 결과 받기</h3><p>현재 결과 링크와 핵심 문제를 이메일로 보내드립니다.</p><input type="email" name="email" required placeholder="you@example.com" aria-label="결과를 받을 이메일"><label class="check"><input type="checkbox" name="marketing"> SuaveForge 개선 사례 및 서비스 안내도 이메일로 받겠습니다.</label><button type="submit">리포트 받기</button><p class="status"></p></form><form class="formbox" id="consultForm"><h3>무료로 직접 봐달라고 하기</h3><p>로그인·예약·결제·관리자처럼 외부에서 확인할 수 없는 영역은 직접 사용해보고 가장 큰 문제를 찾습니다.</p><input name="name" placeholder="이름 / 회사명" aria-label="이름 또는 회사명"><input name="email" type="email" placeholder="이메일" aria-label="이메일"><input name="phone" placeholder="연락처" aria-label="연락처"><textarea name="message" rows="4" placeholder="특히 확인했으면 하는 흐름이 있다면 적어주세요." aria-label="확인 요청 내용"></textarea><button type="submit">무료 컨설팅 요청</button><p class="status"></p></form></div></section>`;
    wire();wireFilters();initCharts(all,cats,[['성능',perf],['SEO',seo],['접근성',a11y]],lh);initPremiumMotion();initTooltips();
  }
    ...
    if(tree){
      ...
      if(all.length<=3){
        ...
        chart.setOption({
          ...
          tooltip:{trigger:'item',backgroundColor:'rgba(8,11,16,.96)',borderColor:'#2b3442',borderWidth:1,textStyle:{color:'#fff',fontSize:12},formatter:p=>{const x=traceItems[p.seriesIndex];return x?`<b>${esc(x.title)}</b><br/>${esc(x.category)} · 우선순위 ${x._p}<br/>수정 위치 ${esc(x._layer)}<br/><span style="color:#9aa5b3">항목을 선택하면 상세 진단으로 이동</span>`:''}},
