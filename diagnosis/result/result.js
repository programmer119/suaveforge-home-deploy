(()=>{
  ...
  async function load(){
    try{
      ...
      if(d.status==='failed'){main.innerHTML='<div class="result-head"><div class="eyebrow">DIAGNOSIS FAILED</div><h1>진단을 완료하지 못했습니다.</h1><p>'+esc(d.error_message||'대상 사이트 응답 또는 검사 환경을 확인해 주세요.')+'</p><a class="btn" href="../">다시 진단하기</a></div>';return}
  ...
  function render(d){
    const top=d.top_issues||[],all=d.findings||[],shots=d.screenshots||{},summary=d.summary?.conclusion||'공개 페이지 검사 결과를 정리했습니다.';
