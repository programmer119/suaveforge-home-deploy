    ...
    if(tree){
      ...
      if(all.length<=3){
        ...
        const lineSeries=traceItems.map((x,i)=>({
          ...
          endLabel:{show:true,formatter:x._layer,color:'#dbe4ef',fontSize:9,fontWeight:800,distance:10},
          label:{show:true,position:'top',formatter:p=>p.dataIndex===1?`P${x._p}`:'',color:'#fff',fontSize:9,fontWeight:900}
        }));
        chart.setOption({
          animationDuration:1200,animationEasing:'cubicOut',
          grid:{left:34,right:118,top:40,bottom:38,containLabel:true},
          tooltip:{trigger:'item',backgroundColor:'rgba(8,11,16,.96)',borderColor:'#2b3442',borderWidth:1,textStyle:{color:'#fff',fontSize:10},formatter:p=>{const x=traceItems[p.seriesIndex];return x?`<b>${esc(x.title)}</b><br/>${esc(x.category)} · Priority ${x._p}<br/>수정 계층 ${esc(x._layer)}<br/><span style="color:#9aa5b3">hover · trace · click</span>`:''}},
          xAxis:{type:'category',boundaryGap:false,data:stageLabels,axisLine:{lineStyle:{color:'#35404d'}},axisTick:{show:false},axisLabel:{color:'#8b98a8',fontSize:9,fontWeight:800,margin:14},splitLine:{show:true,lineStyle:{color:'rgba(255,255,255,.055)',type:'dashed'}}},
          yAxis:{type:'value',min:0,max:100,interval:25,axisLine:{show:false},axisTick:{show:false},axisLabel:{color:'#697687',fontSize:8,formatter:v=>v===0?'':v},splitLine:{lineStyle:{color:'rgba(255,255,255,.055)'}}},
