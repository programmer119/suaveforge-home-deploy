(()=>{
  ...
  function render(d){
    ...
    main.innerHTML=`
      ...
      <div class="report-nav-wrap"><nav class="report-nav" aria-label="진단 결과 바로가기"><a href="#scores">핵심 지표</a><a href="#priority">영향도 지도</a><a href="#matrix">분야별 상태</a>${Object.keys(shots).length?'<a href="#evidence">화면 증거</a>':''}<a href="#details">상세 진단</a><a href="#next">다음 단계</a></nav></div>
      ...
      <section class="report-section report-reveal" id="priority"><div class="section-head"><div><div class="section-kicker">02 · Impact intelligence</div><h2>어디부터 고쳐야 하는지 한 번에</h2></div><p>단순 막대/도넛 대신 우선순위 랭킹과 문제 분포를 서로 연결했습니다. 차트의 항목을 클릭하면 해당 상세 진단으로 이동합니다.</p></div><div class="insight-grid"><div class="insight-card dark"><div class="insight-card-head"><div><h3>Priority impact rail</h3><p>상위 ${Math.min(8,all.length)}개 · 실제 rule priority · 고위험 항목은 pulse로 강조</p></div><span class="report-version">CLICK TO INSPECT</span></div><div class="impact-chart" id="impactChart"></div></div><div class="insight-card"><div class="insight-card-head"><div><h3>Issue constellation</h3><p>분야 × 우선순위 · 점 크기는 실제 문제 위치/증거 수에 따라 변합니다.</p></div></div><div class="category-chart" id="issueMapChart"></div></div></div><div class="action-list">${top.length?top.map(x=>actionCard({...x,_index:all.findIndex(y=>y.title===x.title&&y.category===x.category)})).join(''):'<article class="action-card"><div class="action-score"><strong>0</strong><span>PRIORITY</span></div><div class="action-body"><h3>즉시 우선순위로 분류된 문제가 많지 않습니다.</h3><p>상세 진단과 공개 검사 범위를 확인해 주세요.</p></div></article>'}</div></section>
      ...
      ${Object.keys(shots).length?`<section class="report-section report-reveal" id="evidence"><div class="section-head"><div><div class="section-kicker">04 · Visual evidence</div><h2>실제 화면을 filmstrip처럼 검증</h2></div><p>WebPageTest의 filmstrip 사고방식을 가져와 데스크톱·모바일·문제 위치·개선 예시를 하나의 증거 레일로 묶었습니다.</p></div><div class="evidence-shell"><div class="evidence-toolbar"><h3>Captured evidence</h3><span>HOVER · TILT · OPEN ORIGINAL</span></div><div class="shots">${[['desktop','데스크톱 전체 캡처'],['mobile','모바일 전체 캡처'],['annotated','문제 위치 표시'],['improvement','개선 예시']].filter(([k])=>shots[k]).map(([k,l])=>`<a href="${esc(shots[k])}" target="_blank" rel="noopener" class="evidence-shot"><img src="${esc(shots[k])}" alt="${l}" loading="lazy"><span>${l}</span></a>`).join('')}</div><div class="filmstrip-rail" aria-hidden="true"><i></i><i></i><i></i><i></i></div></div></section>`:''}
  ...
  async function initCharts(all,cats,scores,lh){
    ...
    if(impact){const chart=ech.init(impact);charts.push(chart);const vals=ranked.map(x=>Number(x.priority)||0);chart.setOption({animationDuration:1200,animationDelay:i=>i*70,grid:{left:14,right:36,top:22,bottom:25,containLabel:true},tooltip:{trigger:'axis',axisPointer:{type:'shadow'},backgroundColor:'rgba(7,10,14,.96)',borderColor:'#293341',borderWidth:1,textStyle:{color:'#fff',fontSize:10},formatter:p=>{const x=ranked[p?.[0]?.dataIndex];return x?`<b>${esc(x.title)}</b><br/>Priority ${Math.round(Number(x.priority)||0)} · ${esc(x.category)}<br/><span style="color:#9aa5b3">클릭하여 상세 진단 보기</span>`:''}},xAxis:{type:'value',min:0,max:100,splitNumber:4,axisLabel:{color:'#667180',fontSize:8},axisLine:{show:false},axisTick:{show:false},splitLine:{lineStyle:{color:'#232b35'}},markArea:{silent:true,data:[[{xAxis:0,itemStyle:{color:'rgba(63,109,246,.04)'}},{xAxis:65}],[{xAxis:65,itemStyle:{color:'rgba(213,138,19,.05)'}},{xAxis:85}],[{xAxis:85,itemStyle:{color:'rgba(237,75,67,.07)'}},{xAxis:100}]]}},yAxis:{type:'category',data:ranked.map(x=>short(x.title)),axisTick:{show:false},axisLine:{show:false},axisLabel:{color:'#c6ced8',fontSize:9,width:190,overflow:'truncate'}},series:[{type:'bar',data:vals,barWidth:7,showBackground:true,backgroundStyle:{color:'#242c36',borderRadius:9},itemStyle:{borderRadius:9,color:p=>new ech.graphic.LinearGradient(0,0,1,0,[{offset:0,color:'#334155'},{offset:1,color:priorityColor(p.value)}])},z:2},{type:'scatter',data:ranked.map((x,i)=>[Number(x.priority)||0,i,x._index]),symbolSize:11,itemStyle:{color:p=>priorityColor(p.value[0]),borderColor:'#fff',borderWidth:1,shadowBlur:13,shadowColor:'rgba(255,255,255,.18)'},z:4},{type:'effectScatter',data:ranked.map((x,i)=>[Number(x.priority)||0,i,x._index]).filter(v=>v[0]>=85),symbolSize:9,rippleEffect:{scale:3,brushType:'stroke'},itemStyle:{color:'#ff685f'},z:5}]});chart.on('click',p=>{const findingIndex=Array.isArray(p.value)&&Number.isInteger(p.value[2])?p.value[2]:(ranked[p.dataIndex]?._index);if(Number.isInteger(findingIndex)&&findingIndex>=0)scrollToFinding(findingIndex)})}
    const map=document.getElementById('issueMapChart');
    if(map){const chart=ech.init(map);charts.push(chart);const categories=cats.map(x=>x.category),points=all.map((x,i)=>[Number(x.priority)||0,categories.indexOf(String(x.category||'기타')),evidenceWeight(x),i,x.title,x.category]);chart.setOption({animationDuration:1300,animationDelay:i=>i*55,grid:{left:20,right:20,top:30,bottom:35,containLabel:true},tooltip:{trigger:'item',backgroundColor:'#11151c',borderWidth:0,textStyle:{color:'#fff',fontSize:10},formatter:p=>{const v=p.value;return `<b>${esc(v[4])}</b><br/>${esc(v[5])} · Priority ${v[0]}<br/>증거 밀도 ${v[2]}`}},xAxis:{type:'value',min:0,max:100,axisLabel:{fontSize:8,color:'#89909a'},splitLine:{lineStyle:{color:'#ece8e1'}},axisLine:{show:false},axisTick:{show:false},markArea:{silent:true,data:[[{xAxis:65,itemStyle:{color:'rgba(213,138,19,.05)'}},{xAxis:85}],[{xAxis:85,itemStyle:{color:'rgba(237,75,67,.06)'}},{xAxis:100}]]}},yAxis:{type:'category',data:categories,axisLabel:{fontSize:9,color:'#58606b'},axisLine:{show:false},axisTick:{show:false},splitLine:{lineStyle:{color:'#f0ede7'}}},series:[{type:'scatter',data:points,symbolSize:v=>Math.min(30,10+v[2]*2.4),itemStyle:{color:p=>priorityColor(p.value[0]),opacity:.82,borderColor:'#fff',borderWidth:2,shadowBlur:14,shadowColor:'rgba(16,18,22,.12)'},emphasis:{scale:1.35},z:3},{type:'effectScatter',data:points.filter(v=>v[0]>=85),symbolSize:v=>Math.min(24,8+v[2]*2),rippleEffect:{scale:2.8,brushType:'stroke'},itemStyle:{color:'#ed4b43'},z:4}]});chart.on('click',p=>{const idx=p.value?.[3];if(Number.isInteger(idx))scrollToFinding(idx)})}
    const tree=document.getElementById('categoryTreemap');
    if(tree){
      const chart=ech.init(tree);charts.push(chart);
      const topByCategory=name=>all.filter(x=>String(x.category||'기타')===name).sort((a,b)=>(Number(b.priority)||0)-(Number(a.priority)||0)).slice(0,4);
      const data=cats.map(x=>({
        name:x.category,
        value:Math.max(1,Math.round(x.avg*1.3+x.count*7)),
        itemStyle:{color:x.max>=85?'#ff685f':x.max>=65?'#f4ae3d':'#5a7dff'},
        children:topByCategory(x.category).map(f=>({name:short(f.title,34),value:Math.max(1,Number(f.priority)||0),findingIndex:all.findIndex(y=>y===f),rawTitle:f.title,itemStyle:{color:priorityColor(Number(f.priority)||0)}})),
        meta:{count:x.count,max:x.max,avg:x.avg}
      }));
      chart.setOption({
        backgroundColor:'transparent',
        animationDuration:1350,
        tooltip:{trigger:'item',backgroundColor:'rgba(8,11,16,.96)',borderWidth:0,textStyle:{color:'#fff',fontSize:10},formatter:p=>{const d=p.data||{};if(d.findingIndex>=0)return '<b>'+esc(d.rawTitle||p.name)+'</b><br/>'+esc((p.treePathInfo&&p.treePathInfo[1]&&p.treePathInfo[1].name)||'')+' · Priority '+Math.round(Number(d.value)||0)+'<br/><span style="color:#9aa5b3">클릭하여 상세 진단 보기</span>';return '<b>'+esc(p.name)+'</b><br/>발견 '+(d.meta?.count??'')+'건 · 최고 '+(d.meta?.max??'')+' · 평균 '+(d.meta?.avg??'');}},
        graphic:[{type:'text',left:'center',top:'middle',silent:true,style:{text:['CATEGORY','SIGNAL GALAXY','총 '+cats.length+'개 분야'].join('\n'),fill:'#eef2f7',font:'700 12px Inter, sans-serif',textAlign:'center',lineHeight:18,opacity:.9}}],
        series:[{
          type:'sunburst',
          radius:['20%','86%'],
          sort:null,
          ...
          label:{rotate:'radial',color:'#eef2f7',fontSize:10,overflow:'truncate'},
          itemStyle:{borderWidth:2,borderColor:'#0f1218'},
          levels:[{},
            {r0:'20%',r:'52%',label:{fontSize:12,fontWeight:800,color:'#fff'},itemStyle:{borderWidth:3,borderColor:'#0f1218'}},
            {r0:'54%',r:'86%',label:{fontSize:9,color:'rgba(255,255,255,.86)'},itemStyle:{borderWidth:2,borderColor:'#141922'}}
          ]
        }]
      });
      chart.on('click',p=>{const d=p.data||{};if(Number.isInteger(d.findingIndex)&&d.findingIndex>=0)return scrollToFinding(d.findingIndex);if(p.name)activateCategory(p.name)});
    }
  ...
  function wire(){
    document.querySelectorAll('[data-jump-finding]').forEach(card=>{card.style.cursor='pointer';card.addEventListener('click',e=>{if(e.target.closest('a,button,code'))return;scrollToFinding(Number(card.dataset.jumpFinding))})});
