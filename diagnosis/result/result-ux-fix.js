(()=>{
  ...
  function ensureStyles(){
    ...
    style.textContent=`
      ...
      #priority[data-sf-mode="single"] .issue-spotlight,
      ...
      #priority .sf-severity-tabs button{color:#20242b!important;min-width:auto!important;padding-inline:14px!important}
      #priority .sf-severity-tabs button.active{color:#fff!important}
      #priority .sf-priority-help{position:relative;z-index:2}
      #details .detail-priority.sf-priority-readable{min-width:74px;width:auto;padding-inline:8px;font-size:13px;white-space:nowrap}
  ...
  function fixPriority(){
    ...
    const spotlight=section.querySelector('.issue-spotlight');
    if(spotlight){
      const n=readNumber(spotlight.querySelector(':scope > b'));
      if(n!==null)setText(spotlight.querySelector(':scope > b'),`수정 우선순위 ${Math.round(n)}/100`);
    }

    document.querySelectorAll('#details .detail-priority').forEach(el=>{
      const n=readNumber(el);
      if(n===null)return;
      const [label]=priorityState(n);
      setText(el,`${Math.round(n)}/100`);
      el.classList.add('sf-priority-readable');
    ...
    if(help){
      ...
      const em=help.querySelector('em');
      if(em)setText(em,'사이트 점수가 아니라, 발견된 문제를 먼저 고칠 순서를 0–100으로 나타낸 값입니다.');
    }
    ...
    if(cards.length===1){
      ...
      const card=cards[0];
      const n=readNumber(card.querySelector('.action-score strong'))??0;
      const [label]=priorityState(n);
      ...
      if(!summary){
        ...
        const anchor=section.querySelector('.action-list');
        anchor?.insertAdjacentElement('beforebegin',summary);
      }
      summary.innerHTML=`<div><small>현재 발견 1건</small><strong>${title.replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}</strong><p>현재 진단에서 발견된 문제는 이 1건뿐입니다. 아래에서 근거와 수정 방법을 확인할 수 있습니다.</p></div><div class="sf-single-priority"><b>${Math.round(n)}/100</b><span>${label}</span></div>`;

      const head=section.querySelector('.section-head');
      if(head){
        setText(head.querySelector('h2'),'발견된 문제 1건');
        setText(head.querySelector('p'),`현재 발견된 문제는 1건이며 수정 우선순위는 ${Math.round(n)}/100입니다. 문제 수가 1개라 비교 그래프는 표시하지 않습니다.`);
      }
      if(matrix){
  ...
  function fixDiscoveryRows(){
    document.querySelectorAll('#discovery .discovery-signal').forEach(row=>{
      ...
      row.classList.remove('sf-row-current');
